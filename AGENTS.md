...
- **The Windows installer is per-user and asks for no elevation**, and the reason is what it
  costs to ask. Windows blocked the installer outright on a Windows 11 machine, with no "run
  anyway" offered at all — three different mechanisms do that, and only SmartScreen's *warn*
  mode has the bypass everyone quotes. The elevation was buying nothing: `wails.files` copies
  the app binary and two shortcuts, and the payload and the service are registered later, by
  the app itself, from a shortcut, `asInvoker`. What it did buy was an *Unknown publisher* UAC
  prompt and a third heuristic signal next to "writes executables" and "registers something
  that starts at login". Signing is not the alternative it looks like: Microsoft documents that
  reputation is earned by download volume even with a certificate, and that since March 2024 an
  EV one no longer skips the prompt either.
- **The uninstaller's registry keys are written by `project.nsi`, not by `wails.writeUninstaller`.**
  That macro writes to `HKLM` literally, without ever consulting `REQUEST_EXECUTION_LEVEL`, so
  per-user it fails — and NSIS does not abort on a failed `WriteRegStr`. The install would look
  fine and leave nothing in "Add or remove programs". The block is spelled out against
  `SHELL_CONTEXT` instead, which follows `wails.setShellContext`. `wails_tools.nsh` is
  regenerated by every `wails build` and says so on line 1: it is never the place to fix this.
  Upgrading Wails means diffing that block against the macro it replaces.
